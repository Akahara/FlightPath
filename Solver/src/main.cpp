...
int main()
{
    ...
    //GeoMap map = serializer.parseMap("aerodromes.csv");

    XLSSerializer serializer;
    GeoMap map = serializer.parseMap("aerodromes.xlsx");

    for (const Station &s : map.getStations()) {
        std::cout << s.getOACI() << " | " << s.getName() << " | " << s.getLocation().lon << " | " << s.getLocation().lat << " | " << s.getStatus() << " | " << s.getNightVFR() << " | " << s.getFuel() << std::endl;
    }
